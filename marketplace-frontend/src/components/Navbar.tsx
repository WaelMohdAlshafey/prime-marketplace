                    ...
                    <div className="container mx-auto px-2 sm:px-4 flex items-center justify-between gap-1 sm:gap-2 py-1.5 sm:py-2">
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0 flex-wrap">
                            ...
                            <div className="relative" ref={supportRef}>
                                ...
                                <AnimatePresence>
                                    {supportOpen && (
                                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className={dropdownClasses}>
                                            <Link href="/help" className="block w-full text-right px-3 py-1.5 hover:bg-gray-100 text-xs" onClick={() => setSupportOpen(false)}>{t('help')}</Link>
                                            <Link href="/faq" className="block w-full text-right px-3 py-1.5 hover:bg-gray-100 text-xs" onClick={() => setSupportOpen(false)}>{t('footer.faq')}</Link>
                                            <Link href="/returns" className="block w-full text-right px-3 py-1.5 hover:bg-gray-100 text-xs" onClick={() => setSupportOpen(false)}>{t('footer.returns')}</Link>
                                            <Link href="/shipping" className="block w-full text-right px-3 py-1.5 hover:bg-gray-100 text-xs" onClick={() => setSupportOpen(false)}>{t('footer.shipping')}</Link>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            </div>
                            ...
                            <div className="relative" ref={currencyRef}>
                                ...
                                <AnimatePresence>
                                    {currencyOpen && (
                                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className={`${dropdownClasses} min-w-[80px]`}>
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                            ...
                            <div className="relative" ref={userMenuRef} onClick={(e) => e.stopPropagation()}>
                                <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                                    ...
                                    {user ? (
                                        <>
                                            <span className="text-[10px] sm:text-sm font-medium truncate max-w-[50px] sm:max-w-none">{user.username}</span>
                                ...
                                <AnimatePresence>
                                    {userMenuOpen && (
                                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                                            ...
                                            {!user ? (
                                                <>
                                                    <Link href="/auth/login" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>{t('loginTitle')}</Link>
                                                    <Link href="/auth/register" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>{t('registerTitle')}</Link>
                                                </>
                                            ) : (
                                                <>
                                                    <Link href="/cart" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>🛒 {t('cart')}</Link>
                                                    <Link href="/orders" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>📋 {t('orders')}</Link>
                                                    <Link href="/suggest" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>💡 Suggest</Link>
                                                    {(user.role === 'Vendor' || user.role === 'Admin') && (
                                                        <Link href="/vendor/dashboard" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>📊 {t('dashboard')}</Link>
                                                    )}
                                                    {(user.role === 'Vendor' || user.role === 'Admin' || user.role === 'Employee') && (
                                                        <Link href="/admin/orders" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>📦 Manage Orders</Link>
                                                    )}
                                                    {user.role === 'Admin' && (
                                                        <>
                                                            <Link href="/admin/users" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>👥 {t('users')}</Link>
                                                            <Link href="/admin/products" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>📦 Products</Link>
                                                            <Link href="/admin/pages" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>📄 Pages</Link>
                                                            <Link href="/admin" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>⚙️ {t('admin')}</Link>
                                                            <Link href="/admin/golden-links" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>🔗 Golden Links</Link>
                                                        </>
                                                    )}
                                                    <Link href="/chat" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>💬 Chat</Link>
                                                    <Link href="/profile" className="block px-4 py-2 text-text hover:bg-primary/10 text-sm" onClick={() => setUserMenuOpen(false)}>👤 {t('profile')}</Link>
